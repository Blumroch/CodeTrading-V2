    ...
    for iPourcentageVente in range(1,iIndiceZoneVente+1,1):
     ...
     #On garde seulement ceux qui ont fait des profits     
        if fCrypto*row['close'] > iProfitMin or fUsd > iProfitMin :
            ...
            if fUsd == 0 :            
                myrow = {'iZoneVente': 100-iPourcentageAchat, 'iZoneAchat':100 + iPourcentageVente,'fProfit': round(fUsdTransaction,2),'iTradeBon':itradeBon,'iTradePasBon':itradePasBon,'fRatioTradeBon':round(fRatioTradeBon,2)}
            else :
                myrow = {'iZoneVente': 100-iPourcentageAchat, 'iZoneAchat':100 + iPourcentageVente,'fProfit': round(fUsd,2),'iTradeBon':itradeBon,'iTradePasBon':itradePasBon,'fRatioTradeBon':round(fRatioTradeBon,2)}
