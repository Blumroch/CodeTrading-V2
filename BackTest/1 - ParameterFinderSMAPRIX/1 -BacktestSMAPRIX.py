...
#On fait semblant d'investir 1000 fBTCT à dateDebut
...
for index, row in df.iterrows() :
    #ACHAT
    if row['close'] > row['SMA']  and fBTC > 0 :    
        fBTCTransaction = fBTC # Pour calculer la valeur de la transcation
        fCrypto = fBTC / row['close'] #Converti fBTC en fCrypto comme si on vendait et on enlève les fee
    ...
    #VEND
        ...
        #Converti fBTC en fCrypto comme si on vendait                
        fBTC = fCrypto * row['close']  #Converti les fCrypto en fBTC comme si on vendait et on enlève les fee
...
pourcentageMaintien = ((df.iloc[len(df)-2]['close'] - df.iloc[0]['close'])/df.iloc[0]['close']) * 100 # Performence du maintien

if iTradeBon>0 :
    gainMoyenPositif = fTotalMeilleureTransaction/iTradeBon
else: 
    gainMoyenPositif = 0

if iTradePasBon>0 :    
    gainMoyenNegatif = fTotalPireTransaction/iTradePasBon
else :
    gainMoyenNegatif = 0

print("")
print("Période : [" + str(df.index[0]) + "] -> [" +str(df.index[len(df)-1]) + "]")
print("Montant Départ dans le wallet :",fBTCDepart,"$")
print("Montant final dans le wallet :", round(fMontantFinalWallet,2),"$")
...
print ("Frais total :",round(fTotalFrais,2),"$")
